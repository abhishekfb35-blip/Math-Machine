...
- **Key Pages**:
    ...
    - **Admin Consent** (`/admin/consent`): Manage consent popup settings (headline, description, consent text, discount %, enable/disable) and configurable form fields (First Name, Last Name, Email, Phone — each with Show/Required toggles; Email is locked as always-on). View all collected signups with discount code usage status. Settings stored in `site_config` under key `consent-popup`.
