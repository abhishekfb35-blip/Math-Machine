...
- **Email Delivery Tracking**: Order confirmation emails (customer + admin) have per-recipient delivery tracking. `email_status` text column on `orders` table stores JSON with `customerEmail`/`adminEmail` delivery details (sent, messageId, error), channel, and timestamp. `NotificationResult` returns structured `EmailDeliveryDetail` per email. `OrderService.sendAndSaveEmailStatus()` persists status without blocking checkout. Admin Orders detail view shows an "Email Notifications" card with color-coded badges (green=all sent, red=all failed, yellow=partial, gray=no data).
