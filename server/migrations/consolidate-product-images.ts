...
export async function consolidateProductImages(): Promise<void> {
  ...
  // Count mismatches before fixing so we can log how many were corrected.
  ...
  `);
  const mismatchedBefore = Number((beforeResult.rows[0] as Record<string, unknown>).mismatched ?? 0);

  if (mismatchedBefore > 0) {
    // a) Shift all existing gallery rows up by 1 (makes position 0 vacant for affected products).
    ...
    await db.execute(sql`
      ...
      FROM mismatched m
    `);
    console.log(`[consolidateProductImages] Fixed ${mismatchedBefore} products with wrong primary image (sort_order=0 now matches products.image_url).`);
  }
  ...
  `);
  const remaining = Number((afterResult.rows[0] as Record<string, unknown>).remaining ?? 0);
  if (remaining === 0) {
