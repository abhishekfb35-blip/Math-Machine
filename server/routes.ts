...
`;
  ...
  app.get("/api/admin/export-migration-data", requireAdmin, async (_req, res) => {
    try {
      const { pool } = await import("./db");

      const categories = (await pool.query(`SELECT id, slug, name, description, image_url, sort_order FROM categories`)).rows;
      const products = (await pool.query(`SELECT * FROM products`)).rows;
      const productImages = (await pool.query(`SELECT id, product_id, image_url, sort_order, is_primary, created_at, updated_at FROM product_images`)).rows;
      const productReviews = (await pool.query(`SELECT id, product_id, reviewer_name, rating, title, body, amz_review_date, verified_purchase, created_at FROM product_reviews`)).rows;
      const tags = (await pool.query(`SELECT id, name, description FROM tags`)).rows;
      const siteConfig = (await pool.query(`SELECT id, key, value FROM site_config`)).rows;
      ...
      console.error("Export migration data error:", err);
      res.status(500).json({ message: "Failed to export migration data" });
    }
  });

  app.post("/api/admin/apply-migration", requireAdmin, async (req, res) => {
    try {
      const { pool } = await import("./db");
      const data = req.body;
      ...
      if (data.step) {
        ...
        try {
          ...
          if (data.step === "drop-constraints") {
            await client.query(`ALTER TABLE products DROP CONSTRAINT IF EXISTS products_category_id_categories_id_fk`);
            await client.query(`ALTER TABLE product_images DROP CONSTRAINT IF EXISTS product_images_product_id_products_id_fk`);
            await client.query(`ALTER TABLE product_reviews DROP CONSTRAINT IF EXISTS product_reviews_product_id_products_id_fk`);
            await client.query(`ALTER TABLE cart_items DROP CONSTRAINT IF EXISTS cart_items_product_id_products_id_fk`);
            await client.query(`ALTER TABLE order_items DROP CONSTRAINT IF EXISTS order_items_product_id_products_id_fk`);
            await client.query(`ALTER TABLE product_tags DROP CONSTRAINT IF EXISTS product_tags_product_id_products_id_fk`);
            await client.query(`ALTER TABLE product_tags DROP CONSTRAINT IF EXISTS product_tags_tag_id_tags_id_fk`);
            summary.push("Dropped all foreign key constraints");
          } else if (data.step === "migrate-categories" && data.categories) {
            const catSlugToDevId: Record<string, string> = {};
            for (const cat of data.categories) catSlugToDevId[cat.slug] = cat.id;
            const existing = (await client.query(`SELECT id, slug FROM categories`)).rows;
            let updated = 0;
            for (const ec of existing) {
              const devId = catSlugToDevId[ec.slug];
              if (devId && devId !== ec.id) {
                await client.query(`UPDATE categories SET id = $1 WHERE id = $2`, [devId, ec.id]);
                updated++;
              }
            }
            for (const cat of data.categories) {
              await client.query(
                `UPDATE categories SET name = $1, description = $2, image_url = $3, sort_order = $4 WHERE slug = $5`,
                [cat.name, cat.description, cat.image_url, cat.sort_order, cat.slug]
              );
            }
            summary.push(`Updated ${updated} category IDs and all category fields`);
          } else if (data.step === "migrate-products" && data.products) {
            const existing = (await client.query(`SELECT id, slug FROM products`)).rows;
            const slugToDevData: Record<string, any> = {};
            for (const p of data.products) slugToDevData[p.slug] = p;
            let idUpdated = 0, skuUpdated = 0;
            for (const ep of existing) {
              const dev = slugToDevData[ep.slug];
              if (dev && dev.id !== ep.id) {
                await client.query(`UPDATE products SET id = $1 WHERE id = $2`, [dev.id, ep.id]);
                idUpdated++;
              }
            }
            for (const dev of data.products) {
              const r = await client.query(
                `UPDATE products SET sku = $1, category_id = $2 WHERE slug = $3`,
                [dev.sku, dev.category_id, dev.slug]
              );
              if (r.rowCount && r.rowCount > 0) skuUpdated++;
            }
            summary.push(`Updated ${idUpdated} product IDs, ${skuUpdated} SKUs/category_ids`);
          } else if (data.step === "migrate-images" && data.productImages) {
            await client.query(`DELETE FROM product_images`);
            let inserted = 0;
            for (const img of data.productImages) {
              await client.query(
                `INSERT INTO product_images (id, product_id, image_url, sort_order, is_primary, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                [img.id, img.product_id, img.image_url, img.sort_order, img.is_primary, img.created_at, img.updated_at]
              );
              inserted++;
            }
            summary.push(`Replaced product_images: ${inserted} rows`);
          } else if (data.step === "migrate-reviews" && data.productReviews) {
            await client.query(`DELETE FROM product_reviews`);
            let inserted = 0;
            for (const rev of data.productReviews) {
              await client.query(
                `INSERT INTO product_reviews (id, product_id, reviewer_name, rating, title, body, amz_review_date, verified_purchase, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
                [rev.id, rev.product_id, rev.reviewer_name, rev.rating, rev.title, rev.body, rev.amz_review_date, rev.verified_purchase, rev.created_at]
              );
              inserted++;
            }
            summary.push(`Replaced product_reviews: ${inserted} rows`);
          } else if (data.step === "migrate-tags" && data.tags) {
            await client.query(`DELETE FROM product_tags`);
            await client.query(`DELETE FROM tags`);
            for (const tag of data.tags) {
              await client.query(`INSERT INTO tags (id, name, description) VALUES ($1, $2, $3)`, [tag.id, tag.name, tag.description]);
            }
            summary.push(`Replaced tags: ${data.tags.length} rows`);
          } else if (data.step === "update-references" && data.prodMapping) {
            let oi = 0, ci = 0;
            for (const [oldId, newId] of Object.entries(data.prodMapping)) {
              const r1 = await client.query(`UPDATE order_items SET product_id = $1 WHERE product_id = $2`, [newId, oldId]);
              oi += r1.rowCount || 0;
              const r2 = await client.query(`UPDATE cart_items SET product_id = $1 WHERE product_id = $2`, [newId, oldId]);
              ci += r2.rowCount || 0;
            }
            summary.push(`Updated ${oi} order_items, ${ci} cart_items references`);
          } else if (data.step === "migrate-config" && data.siteConfig) {
            for (const sc of data.siteConfig) {
              await client.query(
                `INSERT INTO site_config (id, key, value) VALUES ($1, $2, $3) ON CONFLICT (key) DO UPDATE SET value = $3`,
                [sc.id, sc.key, sc.value]
              );
            }
            summary.push(`Upserted ${data.siteConfig.length} site_config entries`);
          } else if (data.step === "add-constraints") {
            await client.query(`ALTER TABLE products ADD CONSTRAINT products_category_id_categories_id_fk FOREIGN KEY (category_id) REFERENCES categories(id)`);
            await client.query(`ALTER TABLE product_images ADD CONSTRAINT product_images_product_id_products_id_fk FOREIGN KEY (product_id) REFERENCES products(id)`);
            await client.query(`ALTER TABLE product_reviews ADD CONSTRAINT product_reviews_product_id_products_id_fk FOREIGN KEY (product_id) REFERENCES products(id)`);
            await client.query(`ALTER TABLE cart_items ADD CONSTRAINT cart_items_product_id_products_id_fk FOREIGN KEY (product_id) REFERENCES products(id)`);
            await client.query(`ALTER TABLE order_items ADD CONSTRAINT order_items_product_id_products_id_fk FOREIGN KEY (product_id) REFERENCES products(id)`);
            await client.query(`ALTER TABLE product_tags ADD CONSTRAINT product_tags_product_id_products_id_fk FOREIGN KEY (product_id) REFERENCES products(id)`);
            await client.query(`ALTER TABLE product_tags ADD CONSTRAINT product_tags_tag_id_tags_id_fk FOREIGN KEY (tag_id) REFERENCES tags(id)`);
            summary.push("Re-added all foreign key constraints");
          }

          await client.query("COMMIT");
          return res.json({ success: true, step: data.step, summary });
        } catch (err) {
          await client.query("ROLLBACK");
          return res.status(500).json({ success: false, step: data.step, message: `Step ${data.step} failed: ${(err as Error).message}` });
        } finally {
          client.release();
        }
      }
      ...
      if (!data.categories || !data.products) {
        return res.status(400).json({ message: "Missing migration data" });
      }
      ...
      try {
        ...
        const prodCatSlugToDevId: Record<string, string> = {};
        for (const cat of data.categories) {
          prodCatSlugToDevId[cat.slug] = cat.id;
        }

        const prodSlugToDevData: Record<string, any> = {};
        for (const prod of data.products) {
          prodSlugToDevData[prod.slug] = prod;
        }

        const existingCats = (await client.query(`SELECT id, slug FROM categories`)).rows;
        const catOldToNew: Record<string, string> = {};
        for (const ec of existingCats) {
          const devId = prodCatSlugToDevId[ec.slug];
          if (devId && devId !== ec.id) {
            catOldToNew[ec.id] = devId;
          }
        }

        const existingProds = (await client.query(`SELECT id, slug FROM products`)).rows;
        const prodOldToNew: Record<string, string> = {};
        for (const ep of existingProds) {
          const devData = prodSlugToDevData[ep.slug];
          if (devData && devData.id !== ep.id) {
            prodOldToNew[ep.id] = devData.id;
          }
        }

        summary.push(`Category ID mappings: ${Object.keys(catOldToNew).length}`);
        summary.push(`Product ID mappings: ${Object.keys(prodOldToNew).length}`);
        ...
        await client.query(`ALTER TABLE product_tags DROP CONSTRAINT IF EXISTS product_tags_product_id_products_id_fk`);
        await client.query(`ALTER TABLE product_tags DROP CONSTRAINT IF EXISTS product_tags_tag_id_tags_id_fk`);
        summary.push("Dropped foreign key constraints");

        for (const [oldId, newId] of Object.entries(catOldToNew)) {
          await client.query(`UPDATE categories SET id = $1 WHERE id = $2`, [newId, oldId]);
        }
        summary.push(`Updated ${Object.keys(catOldToNew).length} category IDs`);
        ...
        for (const [oldId, newId] of Object.entries(prodOldToNew)) {
          await client.query(`UPDATE products SET id = $1 WHERE id = $2`, [newId, oldId]);
        }
        summary.push(`Updated ${Object.keys(prodOldToNew).length} product IDs`);

        let skuUpdated = 0;
        for (const devProd of data.products) {
          const result = await client.query(
            `UPDATE products SET sku = $1, category_id = $2 WHERE slug = $3`,
            [devProd.sku, devProd.category_id, devProd.slug]
          );
          if (result.rowCount && result.rowCount > 0) skuUpdated++;
        }
        summary.push(`Updated SKU and category_id for ${skuUpdated} products`);
        ...
        summary.push(`Replaced product_images: ${imgInserted} rows inserted`);
        ...
        summary.push(`Replaced product_reviews: ${revInserted} rows inserted`);
        ...
        if (data.tags && data.tags.length > 0) {
          ...
          summary.push(`Replaced tags: ${data.tags.length} rows inserted`);
        }

        let orderItemsUpdated = 0;
        for (const [oldId, newId] of Object.entries(prodOldToNew)) {
          const r = await client.query(`UPDATE order_items SET product_id = $1 WHERE product_id = $2`, [newId, oldId]);
          orderItemsUpdated += r.rowCount || 0;
        }
        summary.push(`Updated ${orderItemsUpdated} order_items product references`);

        let cartItemsUpdated = 0;
        for (const [oldId, newId] of Object.entries(prodOldToNew)) {
          const r = await client.query(`UPDATE cart_items SET product_id = $1 WHERE product_id = $2`, [newId, oldId]);
          cartItemsUpdated += r.rowCount || 0;
        }
        summary.push(`Updated ${cartItemsUpdated} cart_items product references`);
        ...
        await client.query(`ALTER TABLE product_tags ADD CONSTRAINT product_tags_product_id_products_id_fk FOREIGN KEY (product_id) REFERENCES products(id)`);
        await client.query(`ALTER TABLE product_tags ADD CONSTRAINT product_tags_tag_id_tags_id_fk FOREIGN KEY (tag_id) REFERENCES tags(id)`);
        summary.push("Re-added foreign key constraints");
        ...
        await client.query("COMMIT");
        summary.push("Migration committed successfully");

        res.json({ success: true, summary });
      } catch (err) {
        await client.query("ROLLBACK");
        console.error("Migration error:", err);
        res.status(500).json({ success: false, message: `Migration failed and was rolled back: ${(err as Error).message}`, summary });
      } finally {
        client.release();
      }
    } catch (err) {
      console.error("Migration setup error:", err);
      res.status(500).json({ message: "Failed to set up migration" });
    }
  });
